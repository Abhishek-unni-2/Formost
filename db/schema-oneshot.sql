-- ============================================================
--  FORMOST OPS — schema + seed as ONE statement
--  For query consoles that reject multi-statement scripts
--  (e.g. the Vercel Storage "Query" box). Turn OFF "Read-only".
--  The canonical multi-statement version is db/schema.sql.
-- ============================================================
DO $$
BEGIN
  ...
  CREATE TABLE IF NOT EXISTS users (
    id serial PRIMARY KEY, name text NOT NULL, role text NOT NULL, pass text NOT NULL,
    district_id integer DEFAULT 0, phone text DEFAULT '', email text DEFAULT '', photo text DEFAULT ''
  );
  CREATE TABLE IF NOT EXISTS steps (
    num text PRIMARY KEY, icon text DEFAULT 'fa-circle-dot', title text NOT NULL,
    role text DEFAULT '', dept text DEFAULT '', description text DEFAULT ''
  );
  CREATE TABLE IF NOT EXISTS leads (
    id serial PRIMARY KEY, name text NOT NULL, phone text DEFAULT '', location text DEFAULT '',
    system text DEFAULT '', source text DEFAULT '', stage integer DEFAULT 1, status text DEFAULT 'pending',
    notes text DEFAULT '', date text DEFAULT '', district_id integer DEFAULT 1
  );
  CREATE TABLE IF NOT EXISTS quotations (
    id serial PRIMARY KEY, customer_name text NOT NULL, place text DEFAULT '', phone text DEFAULT '',
    kw text DEFAULT '3', amount integer DEFAULT 0, date text DEFAULT '', notes text DEFAULT '',
    created_by text DEFAULT '', district_id integer DEFAULT 1
  );
  ...
  INSERT INTO districts (id, name, location) VALUES
    (1, 'Malappuram', 'Tirur'), (2, 'Kozhikode', 'Calicut'),
    (3, 'Thrissur', 'Thrissur'), (4, 'Ernakulam', 'Kochi')
  ...
  PERFORM setval(pg_get_serial_sequence('districts', 'id'), GREATEST((SELECT MAX(id) FROM districts), 1));
  PERFORM setval(pg_get_serial_sequence('users', 'id'),     GREATEST((SELECT MAX(id) FROM users), 1));
  PERFORM setval(pg_get_serial_sequence('leads', 'id'),      GREATEST((SELECT MAX(id) FROM leads), 1));
  PERFORM setval(pg_get_serial_sequence('quotations', 'id'), GREATEST((SELECT COALESCE(MAX(id), 0) FROM quotations), 1));
END $$;
